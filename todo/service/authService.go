...
func getUserDetail(email_username string) (types.User, int, error) {
	...
	if dbErr != nil {
		logger.Logger(dbErr.Error(), "getUserDetail")
...
func checkIfUserExistByEmailOrUsername(email_username string) (bool, int, error) {
	...
	if dbErr != nil {
		logger.Logger(dbErr.Error(), "checkIfUserExistByEmailOrUsername")
...
func checkIfUserExist(username string, email string) (bool, int, error) {
	...
	if dbErr != nil {
		logger.Logger(dbErr.Error(), "checkIfUserExist")
...
func verifyUserPassword(loginData types.LoginSchema) (types.User, int, error) {
	...
	if dbErr != nil {
		logger.Logger(dbErr.Error(), "verifyUserPassword")
	...
	passwordMatched := utils.CheckPasswordHash(loginData.Password, userPassword)
...
func changeUserPasswordService(email string, password string, request_type int) (bool, int, error) {
	...
	if checkRequestErr != nil {
		logger.Logger(checkRequestErr.Error(), "changeUserPasswordService")
		return false, http.StatusInternalServerError, errors.New("something went wrong")
	}
	if !isRequestExist {
	...
	if hashErr != nil {
		logger.Logger(hashErr.Error(), "changeUserPasswordService")
	...
	if updatePasswordErr != nil {
		logger.Logger(updatePasswordErr.Error(), "changeUserPasswordService")
		return false, http.StatusInternalServerError, errors.New("internal server error")
	}
...
func LogoutService(userId int, token string) (bool, int, error) {
	...
	if dbErr != nil {
		logger.Logger(dbErr.Error(), "LogoutService")
...
func LogoutFromAllDeviceService(userId int) (bool, int, error) {
	...
	if dbErr != nil {
		logger.Logger(dbErr.Error(), "LogoutFromAllDeviceService")
		if strings.Contains(dbErr.Error(), "Invalid id") {
			return false, http.StatusBadRequest, errors.New("invalid id")
		}
		return false, http.StatusInternalServerError, errors.New("something went wrong")
...
func LoginService(loginData types.LoginSchema) (types.User, string, int, error) {
	...
	token, tokenErr := utils.SaveTokenInDb(userData.Id)
	if tokenErr != nil {
...
func RegisterService(registerData types.RegisterSchema) (types.UserRegisterStruct, int, error) {
	...
	existingOTP, otpErr := utils.CheckIfOtpAlreadyExist(registerData.Email, REGISTER)
	...
	if doesOTPExist {
		...
		var dbErr error
		otp, dbErr = utils.CreateAndSaveOTP(registerData.Email, REGISTER)
		if dbErr != nil {
			return types.UserRegisterStruct{}, http.StatusInternalServerError, errors.New("something went wrong")
		}
	}

	err := publisher.AddEmailTask(registerData.Email, "Email Verification", "Email Verifucation\nOTP : "+strconv.Itoa(otp))

	if err != nil {
		logger.Logger("SMTP ERROR : "+err.Error(), "RegisterService")
	}
...
func VerifyOtpAndCompleteRegistration(data types.VerifyOtpAndRegisterSchema) (types.User, string, int, error) {
	...
	if dbErr != nil {
		logger.Logger(dbErr.Error(), "VerifyOtpAndCompleteRegistration")
		if strings.Contains(dbErr.Error(), "Invalid id") {
	...
	password, hashErr := utils.HashPassword(data.Password)

	if hashErr != nil {
		logger.Logger(hashErr.Error(), "VerifyOtpAndCompleteRegistration")
		return types.User{}, "", http.StatusInternalServerError, errors.New("something went wrong")
	}
	...
	if createErr != nil {
		logger.Logger("CREATE USER DB ERR : "+createErr.Error(), "VerifyOtpAndCompleteRegistration")
		return types.User{}, "", http.StatusInternalServerError, errors.New("unable to create user. Try again")
	}

	token, tokenErr := utils.SaveTokenInDb(userId)
	if tokenErr != nil {
		return types.User{}, "", http.StatusInternalServerError, errors.New("something went wrong")
	}
...
func CreateOTPRequest(email string, requestTypeCode int) (int, error) {
	...
	existingOTP, otpErr := utils.CheckIfOtpAlreadyExist(email, requestTypeCode)
	...
	if doesOTPExist {
		...
		var dbErr error
		otp, dbErr = utils.CreateAndSaveOTP(email, requestTypeCode)
		if dbErr != nil {
			return http.StatusInternalServerError, errors.New("something went wrong")
		}
	}
	...
	err := publisher.AddEmailTask(email, subject, msgBody)

	if err != nil {
		logger.Logger("SMTP ERROR : "+err.Error(), "CreateOTPRequeset")
	}
...
func VerifyOTP(email string, otp int, requestType int) (int,error) {
	...
	if dbErr != nil {
		logger.Logger(dbErr.Error(),"VerifyOTP")
		if strings.Contains(dbErr.Error(), "Invalid id") {
	...
	if createErr != nil {
		logger.Logger(createErr.Error(),"VerifyOTP")
		return http.StatusInternalServerError, errors.New("something went wrong")
	}
...
func ResetPasswordService(userDetail types.User, currentToken string, password string) (int, error) {
	...
	if changePasswordErr != nil {
		logger.Logger(changePasswordErr.Error(), "ResetPasswordService")
	...
	if dbErr != nil {
		logger.Logger(dbErr.Error(), "ResetPasswordService")
		if strings.Contains(dbErr.Error(), "Invalid id") {
...
func DeleteAccountService(userId int, email string, otp int) (bool, int, error) {
	...
	if dbErr != nil {
		logger.Logger(dbErr.Error(), "DeleteAccountService")
		if strings.Contains(dbErr.Error(), "Invalid id") {
	...
	if deleteDbErr != nil {
		logger.Logger(deleteDbErr.Error(), "DeleteAccountService")
...
func UpdateProfileService(profileData types.ProfileSchema, userData types.User) (types.User, int, error) {
	...
	if existErr != nil {
		logger.Logger(existErr.Error(),"UpdateProfileService")
		return types.User{}, http.StatusInternalServerError, errors.New("something went wrong")
	}
	...
	if profileData.Email != "" {
		...
		if err != nil {
			logger.Logger(err.Error(),"UpdateProfileService")
