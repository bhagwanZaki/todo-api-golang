...
func CheckIfOtpAlreadyExist(email string, requestType int) (types.UserOTPDbStruct, error) {
	...
	if checkErr != nil {
		logger.Logger(checkErr.Error(), "CheckIfOtpAlreadyExist")
...
func CreateAndSaveOTP(email string, request_type int) (int, error) {
	...
	if dbErr != nil {
		logger.Logger(dbErr.Error(), "CreateAndSaveOTP")
		return 0, dbErr
	}
...
func CreateToken() (string, string, string, error) {
	bytes, err := generateBytes(env.AUTH_TOKEN_LENGTH / 2)
	if err != nil {
		return "", "", "", err
	}
	token := hex.EncodeToString(bytes)
	digest := hashToken(token)
	tokenKey := token[:env.TOKEN_KEY_LENGTH]
...
func SaveTokenInDb(id int) (string, error) {
	...
	if tokenErr != nil {
		logger.Logger(tokenErr.Error(), "SaveTokenInDb")
	...
	if tokenDbErr != nil {
		logger.Logger(tokenDbErr.Error(), "SaveTokenInDb")
...
func CheckTokenValidity(r *http.Request) (string, error) {
	...
	if len(token) != env.AUTH_TOKEN_LENGTH {
		return "", errors.New("invalid token")
	}

	dbToken, dbErr := GetToken(token[:env.TOKEN_KEY_LENGTH])
	...
	if hmac.Equal([]byte(dbToken.Digest), []byte(hashedDigest)) {
		return token[:env.TOKEN_KEY_LENGTH], nil
	}
...
func GetUserDataFromToken(r *http.Request) (types.User, error) {
	...
	if dbErr != nil {
		logger.Logger(dbErr.Error(),"GetUserDataFromToken")
...
func GetUserDataWithTokenFromToken(r *http.Request) (types.User, string, error) {
	...
	if dbErr != nil {
		logger.Logger(dbErr.Error(),"GetUserDataWithTokenFromToken")
		return types.User{}, "", dbErr
	}
